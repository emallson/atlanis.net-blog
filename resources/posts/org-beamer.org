...
* What is Org-mode?
  ...
  - Structured editing
    ...
    comes loaded with hotkeys, syntax highlighting, outline folding, and a bevy
    of other features.

    #+ATTR_HTML: :width 200 :class img-center
    [[http://media.atlanis.net/blog/org-mode-screen.jpg][http://media.atlanis.net/blog/org-mode-screen.jpg]]
  ...
  - HTML export
    ...
    way as LaTeX is. In fact, this blog post was written as an Org document and
  ...
  Of course, there are /many/ other things it can do. I've only barely
  scratched the surface of its capabilities myself. However, these functions
  are sufficient for our usage: writing Beamer presentations in Org-mode.
...
** So...What's My Workflow?
   ...
   This is all well and good, but John asked about my /workflow/, not how to
   use Org mode! My workflow is simple:

   - Emacs open on one monitor, Evince open on the other
   ...
   quite as simple as a WYSIWYG editor like PowerPoint, it integrates much
   better into my usual development environment.

* But Can I Use Org-mode for Presentations Without Emacs?

  In theory, yes. The format has parsers written in other languages (Common
  Lisp, PHP and Python are all listed on the [[http://orgmode.org/worg/org-tools/][community tools page]]) and there
  are =vim= plugins for Org files ([[https://github.com/hsitz/VimOrganizer][VimOrganizer]] and [[https://github.com/jceb/vim-orgmode][vim-orgmode]]).

  In practice, not really. The parsers are geared towards reading org documents
  into data structures for manipulation, and the vim plugins implement only a
  subset of the functionality. If you want to use Org mode, it is best to use
  Emacs along with it.

  If you're a vim user on the road to recovery, you should check out [[https://bling.github.io/blog/2013/10/27/emacs-as-my-leader-vim-survival-guide/][bling's
  Emacs->Vim Survival Guide]] (also his [[https://bling.github.io/blog/2013/09/09/vim-in-emacs-bootstrap/][Emacs bootstrapping post]]).

* Example Time!

  Of course, all of this is moot unless you can actually generate
  decent-looking presentations with it! That is the goal, right? Since I've
  done a some presentations recently, I'll pull two from my stock.
  ...
  - Birds of a Feather Clojure Code
    ...
    the slides can be downloaded [[http://media.atlanis.net/blog/org-beamer-example-output/bof.pdf][here]]. This presentation demonstrates both
    ...
    copying it into the org file.

  - Keeping Current Clojure Presentation

    I also wrote [[http://media.atlanis.net/blog/clojure-presentation.org][slides]] (and [[https://github.com/emallson/comment-example][code]], but not included in the presentation) for a
    Keeping Current presentation. The PDF output of the slides can be
    downloaded [[http://media.atlanis.net/blog/org-beamer-example-output/keeping-current.pdf][here]]. This Org file demonstrates inline LaTeX (optional
  ...
  available when they were written. You can see many better-looking examples by
  searching for =beamer presentation= on [[https://www.google.com/search?q=beamer+presentation&tbm=isch][Google Images]].
  
* So Should I Use Org-mode?
  
  Yes! At least give it a try! Org-mode has come bundled with Emacs since
  version 22, but you can also install the latest version within Emacs via =M-x
  ...
  many cases superior alternative.
